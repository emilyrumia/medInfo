...
class TemplateCode(object):
    ...
    def generate_tree(self, level="c_class"):
        stat_list_node = TreeFragment(
            self.writer.getvalue(), level=level, pipeline=[NormalizeTree(None)],
...
def generate_repr_code(code, repr, node, fields):
    ...
    code.add_code_line(
        u'        name = getattr(type(self), "__qualname__", type(self).__name__)'
    )
...
def generate_hash_code(code, unsafe_hash, eq, frozen, node, fields):
    ...
    code.add_code_lines(
        ["def __hash__(self):", "    return hash((%s))" % hash_tuple_items,]
    )
