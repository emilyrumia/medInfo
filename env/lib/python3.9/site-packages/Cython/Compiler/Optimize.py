...
class IterationTransform(Visitor.EnvTransform):
    ...
    def _transform_indexable_iteration(
        ...
        target_assign = Nodes.CompilerDirectivesNode(
            target_assign.pos, directives=new_directives, body=target_assign,
        )
        ...
        if len(pos_args) != 1:
            if len(pos_args) == 0:
                return ExprNodes.UnicodeNode(
                    node.pos, value=EncodedString(), constant_result=u""
                )
            return node
        arg = pos_args[0]
    ...
    Pyx_BinopInt_func_types = dict(
        (
            ...
            PyrexTypes.CFuncType(
                ...
                if ret_type.is_pyobject
                else ret_type.exception_value,
            ),
        )
            ...
            if s == u"%%":
                substrings.append(
                    ExprNodes.UnicodeNode(
                        pos, value=EncodedString(u"%"), constant_result=u"%"
                    )
                )
                continue
            if s[0] != u"%":
