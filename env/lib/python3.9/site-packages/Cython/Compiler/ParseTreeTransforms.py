...
class InterpretCompilerDirectives(CythonTransform):
    ...
    def _check_valid_cython_module(self, pos, module_name):
        ...
        error(
            ...
            % (module_name, ". " if extra else "", extra,),
        )
    ...
    def visit_FromCImportStatNode(self, node):
        module_name = node.module_name
        if module_name == u"cython.cimports" or module_name.startswith(
            u"cython.cimports."
        ):
    ...
    def _create_cimport_from_import(self, node_pos, module_name, level, imported_names):
        if module_name == u"cython.cimports" or module_name.startswith(
            ...
            module_name = EncodedString(
                module_name[len(u"cython.cimports.") :]
            )  # may be empty
    ...
    def visit_WithStatNode(self, node):
        ...
        node.body = Nodes.TryFinallyStatNode(
            ...
            body=Nodes.TryExceptStatNode(
                pos, body=body, except_clauses=[except_clause], else_clause=None,
            ),
...
class CalculateQualifiedNamesTransform(EnvTransform):
    ...
    def generate_assignment(self, node, name, value):
        ...
        node.body.stats.insert(
            0,
            Nodes.SingleAssignmentNode(node.pos, lhs=lhs, rhs=rhs,).analyse_expressions(
                self.current_env()
            ),
        )
    ...
    def visit_InPlaceAssignmentNode(self, node):
        ...
        def side_effect_free_reference(node, setting=False):
            if node.is_name:
                ...
                return (
                    ExprNodes.IndexNode(node.pos, base=base, index=index),
                    temps + [index],
                )
    ...
    def visit_SimpleCallNode(self, node):
        ...
        if function:
            if function in InterpretCompilerDirectives.unop_method_nodes:
                if len(node.args) != 1:
                    error(
                        node.function.pos, u"%s() takes exactly one argument" % function
                    )
                else:
                ...
                if len(node.args) != 2:
                    error(
                        node.function.pos,
                        u"%s() takes exactly two arguments" % function,
                    )
                else:
