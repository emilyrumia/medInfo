    ...
    with local_errors(ignore=True):
        ...
        try:
            declaration = TreeFragment(
                u"sizeof(%s)" % name, name=pos[0].filename, initial_pos=pos
            )
        except CompileError:
            pass
        else:
...
class IteratorNode(ScopedExprNode):
    ...
    def generate_result_code(self, code):
        ...
        if is_builtin_sequence or self.may_be_a_sequence:
            code.putln(
                "%s = %s; __Pyx_INCREF(%s);"
                % (self.result(), self.sequence.py_result(), self.result(),)
            )
        ...
        if base_type:
            if not self.start and not self.stop:
                ...
                slice_node = SliceNode(
                    self.pos, start=none_node, stop=none_node, step=none_node,
                )
                return PyrexTypes.MemoryViewSliceType(
...
class AttributeNode(ExprNode):
    ...
    def as_cython_attribute(self):
        if (
            ...
            and not self.attribute == u"parallel"
        ):
    ...
    def generate_deletion_code(self, code, ignore_nonexisting=False):
        ...
        if self.is_py_attr or (
            self.entry.scope.is_property_scope
...
class Py3ClassNode(ExprNode):
    ...
    def analyse_annotations(self, env):
        ...
        if dict_items:
            ...
            lhs = NameNode(
                position, name=StringEncoding.EncodedString(u"__annotations__")
            )
...
class CythonArrayNode(ExprNode):
    ...
    def generate_result_code(self, code):
        ...
        code.putln(
            ...
            % (format_temp, type_info, code.error_goto_if_null(format_temp, self.pos),)
        )
