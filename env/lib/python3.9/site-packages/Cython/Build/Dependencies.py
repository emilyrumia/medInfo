    ...
    while True:
        ...
        # Try to close the quote.
            ...
            if code[q] == quote_type and (
                quote_len == 1
                or (code_len > q + 2 and quote_type == code[q + 1] == code[q + 2])
            ):
                counter += 1
    ...
    for m in dependency_regex.finditer(source):
        ...
        if cimport_from:
            ...
            if m_after_from:
                ...
                cimports.extend(
                    "{0}.{1}".format(cimport_from, s.strip())
    ...
    def included_files(self, filename):
        ...
        for include in self.parse_dependencies(filename)[1]:
            ...
            if include_path:
                ...
                print(
                    u"Unable to locate '%s' referenced from '%s'" % (filename, include)
                )
        return all
        ...
        if isinstance(pattern, str):
            ...
            if cython_sources:
                ...
                if len(cython_sources) > 1:
                    print(
                        u"Warning: Multiple cython sources found for extension '%s': %s\n"
                        u"See https://cython.readthedocs.io/en/latest/src/userguide/sharing_declarations.html "
                        u"for sharing declarations among Cython files."
            ...
            if module_name not in seen:
                ...
                if file not in m.sources:
                    ...
                    try:
                        ...
                        print(
                            u"Warning: Cython source file not found in sources list, adding %s"
                            % file
                        )
            ...
            if ext in (".pyx", ".py"):
                ...
                if force or c_timestamp < dep_timestamp:
                    if not quiet and not force:
                        if source == dep:
                            print(
                                u"Compiling %s because it changed."
                                % Utils.decode_filename(source)
                            )
                        else:
                            print(
                                u"Compiling %s because it depends on %s."
                                % (
                                    Utils.decode_filename(source),
...
def fix_windows_unicode_modules(module_list):
    ...
    for m in module_list:
        ...
        m.export_symbols = make_filtered_list(
            "PyInit_" + m.name.rsplit(".", 1)[-1], m.export_symbols,
        )
        ...
        if len(artifacts) == 1:
            ...
            with contextlib.closing(open(c_file, "rb")) as f:
                with contextlib.closing(
                    gzip_open(fingerprint_file + ".tmp", "wb")
                ) as g:
...
def cleanup_cache(cache, target_size, ratio=0.85):
    try:
        p = subprocess.Popen(
            ["du", "-s", "-k", os.path.abspath(cache)], stdout=subprocess.PIPE
        )
