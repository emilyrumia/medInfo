...
class BM25ABC(interfaces.TransformationABC, metaclass=ABCMeta):
    ...
    def __init__(self, corpus=None, dictionary=None):
        ...
        if dictionary:
            if corpus:
                logger.warning(
                    "constructor received both corpus and dictionary; ignoring the corpus"
                )
    ...
    def __getitem__(self, bow):
        ...
        vector = [
            (term_id, float(weight)) for term_id, weight in zip(term_ids, term_weights)
        ]
        return vector
    ...
    def get_term_weights(self, num_tokens, term_frequencies, idfs):
        term_weights = idfs * (
            ...
            * (self.k1 + 1)
            / (
                term_frequencies
                + self.k1 * (1 - self.b + self.b * num_tokens / self.avgdl)
            )
        )
        return term_weights
