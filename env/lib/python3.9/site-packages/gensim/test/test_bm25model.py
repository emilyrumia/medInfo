...
class OkapiBM25ModelTest(unittest.TestCase):
    ...
    def test_idfs_from_corpus(self):
        ...
        model = OkapiBM25Model(
            corpus=corpus, k1=self.k1, b=self.b, epsilon=self.epsilon
        )
    ...
    def test_idfs_from_dictionary(self):
        model = OkapiBM25Model(
            dictionary=self.dictionary, k1=self.k1, b=self.b, epsilon=self.epsilon
        )
    ...
    def test_score(self):
        ...
        def get_expected_weight(word):
            ...
            numerator = self.k1 + 1
            denominator = 1 + self.k1 * (
                1 - self.b + self.b * len(first_document) / model.avgdl
            )
            return idf * numerator / denominator

        expected_dog_weight = (
            get_expected_weight("dog") if "dog" in first_document else 0.0
        )
        expected_cat_weight = (
            get_expected_weight("cat") if "cat" in first_document else 0.0
        )
        expected_mouse_weight = (
            get_expected_weight("mouse") if "mouse" in first_document else 0.0
        )
        expected_lion_weight = (
            get_expected_weight("lion") if "lion" in first_document else 0.0
        )
