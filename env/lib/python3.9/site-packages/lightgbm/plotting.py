...
def plot_split_value_histogram(
    ...
    title: Optional[
        str
    ] = "Split value histogram for feature with @index/name@ @feature@",
...
def plot_metric(
    ...
    if MATPLOTLIB_INSTALLED:
        ...
        raise ImportError(
            "You must install matplotlib and restart your session to plot metric."
        )
...
def _determine_direction_for_numeric_split(
    fval: float, threshold: float, missing_type_str: str, default_left: bool,
) -> str:
        ...
        if "split_index" in root:  # non-leaf
            ...
            if root["decision_type"] == "==":
                ...
                if len(category_values) > max_category_values:
                    ...
                    threshold = (
                        "||".join(category_values[:2]) + "||...||" + category_values[-1]
                    )
...
def create_tree_digraph(
    ...
    **kwargs
    ...
    if example_case is not None:
        if (
            not isinstance(example_case, (np.ndarray, pd_DataFrame))
            or example_case.ndim != 2
        ):
            raise ValueError(
                "example_case must be a numpy 2-D array or a pandas DataFrame"
            )
...
def plot_tree(
    ...
    **kwargs
    ...
    if MATPLOTLIB_INSTALLED:
        ...
        raise ImportError(
            "You must install matplotlib and restart your session to plot tree."
        )
