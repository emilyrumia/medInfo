...
_PredictionDtype = Union[
    Type[np.float32], Type[np.float64], Type[np.int32], Type[np.int64]
]
...
def _assign_open_ports_to_workers(
    client: Client, workers: List[str],
    ...
    for worker in workers:
        worker_to_future[worker] = client.submit(
            _acquire_port, workers=[worker], allow_other_workers=False, pure=False,
        )
    ...
    for worker, socket_future in worker_to_future.items():
        worker_to_socket_future[worker] = client.submit(
            operator.itemgetter(0), socket_future
        )
        worker_to_port_future[worker] = client.submit(
            operator.itemgetter(1), socket_future
        )
...
def _train_part(
    ...
    **kwargs: Any,
    ...
    if n_evals:
        has_eval_sample_weight = any(
            x.get("eval_sample_weight") is not None for x in list_of_parts
        )
        has_eval_init_score = any(
            x.get("eval_init_score") is not None for x in list_of_parts
        )
        ...
        for i in range(n_evals):
            ...
            x_e, y_e, w_e, init_score_e, g_e = _remove_list_padding(
                x_e, y_e, w_e, init_score_e, g_e
            )
            if x_e:
        ...
        eval_component_idx = [
            i for i in range(n_evals) if i not in missing_eval_component_idx
        ]
        if eval_names:
            local_eval_names = [eval_names[i] for i in eval_component_idx]
        if eval_class_weight:
            kwargs["eval_class_weight"] = [
                eval_class_weight[i] for i in eval_component_idx
            ]
...
def _machines_to_worker_map(
    machines: str, worker_addresses: Iterable[str]
) -> Dict[str, int]:
    ...
    for address in worker_addresses:
        ...
        if not worker_host:
            raise ValueError(
                f"Could not parse host name from worker address '{address}'"
            )
...
def _train(
    ...
    * ``timeout``: time in minutes to wait before closing unused sockets.
    ...
    if eval_set:
        ...
        eval_sets: Dict[
            ...
            List[
                Union[
                    _DatasetNames,
                    Tuple[
                        List[Optional[_DaskMatrixLike]], List[Optional[_DaskVectorLike]]
                    ],
                ]
            ],
        ] = defaultdict(list)
        if eval_sample_weight:
            eval_sample_weights: Dict[
                int, List[Union[_DatasetNames, List[Optional[_DaskVectorLike]]]]
            ] = defaultdict(list)
        if eval_group:
            eval_groups: Dict[
                int, List[Union[_DatasetNames, List[Optional[_DaskVectorLike]]]]
        ...
        if eval_init_score:
            eval_init_scores: Dict[
                int, List[Union[_DatasetNames, List[Optional[_DaskMatrixLike]]]]
            ] = defaultdict(list)
        ...
        for i, (X_eval, y_eval) in enumerate(eval_set):
            ...
            if eval_sample_weight:
                if eval_sample_weight[i] is sample_weight:
                    for parts_idx in range(n_parts):
                        eval_sample_weights[parts_idx].append(
                            _DatasetNames.SAMPLE_WEIGHT
                        )
                else:
                    eval_w_parts = _split_to_parts(
                        data=eval_sample_weight[i], is_matrix=False
                    )
            ...
            if eval_init_score:
                if eval_init_score[i] is init_score:
                    ...
                    eval_init_score_parts = _split_to_parts(
                        data=eval_init_score[i], is_matrix=False
                    )
                    for j in range(n_largest_eval_parts):
                        if j < n_this_eval_parts:
    ...
    params = _choose_param_value(
        main_param_name="local_listen_port", params=params, default_value=12400
    )
    ...
    if machines is not None:
        ...
        worker_address_to_port = _machines_to_worker_map(
            machines=machines, worker_addresses=worker_addresses
        )
    else:
        if listen_port_in_params:
            ...
            worker_address_to_port = {
                address: local_listen_port for address in worker_addresses
            }
        else:
            ...
            (
                worker_to_socket_future,
                worker_address_to_port,
            ) = _assign_open_ports_to_workers(client, list(worker_map.keys()))

        machines = ",".join(
            [
                f"{urlparse(worker_address).hostname}:{port}"
                for worker_address, port in worker_address_to_port.items()
            ]
        )
    ...
    **kwargs
    ...
    if isinstance(data, dask_DataFrame):
        ...
        return data.map_blocks(
            predict_fn, chunks=chunks, meta=pred_row, dtype=dtype, **map_blocks_kwargs,
        )
    else:
        raise TypeError(
            f"Data must be either Dask Array or Dask DataFrame. Got {type(data).__name__}."
        )
