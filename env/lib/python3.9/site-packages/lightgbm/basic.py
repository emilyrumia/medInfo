...
_ctypes_int_ptr = Union[
    "ctypes._Pointer[ctypes.c_int32]", "ctypes._Pointer[ctypes.c_int64]"
]
...
_ctypes_float_ptr = Union[
    "ctypes._Pointer[ctypes.c_float]", "ctypes._Pointer[ctypes.c_double]"
]
...
_LGBM_GroupType = Union[List[float], List[int], np.ndarray, pd_Series]
_LGBM_PositionType = Union[np.ndarray, pd_Series]
_LGBM_InitScoreType = Union[
    List[float], List[List[float]], np.ndarray, pd_Series, pd_DataFrame,
]
...
_LGBM_LabelType = Union[List[float], List[int], np.ndarray, pd_Series, pd_DataFrame]
_LGBM_PredictDataType = Union[
    str, Path, np.ndarray, pd_DataFrame, dt_DataTable, scipy.sparse.spmatrix
]
_LGBM_WeightType = Union[List[float], List[int], np.ndarray, pd_Series]
...
def _get_sample_count(total_nrow: int, params: str) -> int:
    ...
    _safe_call(
        _LIB.LGBM_GetSampleCount(
            ctypes.c_int32(total_nrow), _c_str(params), ctypes.byref(sample_cnt),
        )
    )
...
def _cast_numpy_array_to_dtype(
    array: np.ndarray, dtype: "np.typing.DTypeLike"
) -> np.ndarray:
    ...
    def _get_all_param_aliases() -> Dict[str, List[str]]:
        ...
        _safe_call(
            _LIB.LGBM_DumpParamAliases(
                ctypes.c_int64(buffer_len), ctypes.byref(tmp_out_len), ptr_string_buffer
            )
        )
...
def _load_pandas_categorical(
    file_name: Optional[Union[str, Path]] = None, model_str: Optional[str] = None
) -> Optional[List[List]]:
...
class _InnerPredictor:
    ...
    def __init__(
        ...
        _safe_call(
            _LIB.LGBM_BoosterGetNumClasses(self._handle, ctypes.byref(out_num_class))
        )
        self.num_class = out_num_class.value

    @classmethod
    def from_booster(
        cls, booster: "Booster", pred_parameter: Dict[str, Any]
    ) -> "_InnerPredictor":
        ...
        _safe_call(
            _LIB.LGBM_BoosterGetCurrentIteration(
                booster._handle, ctypes.byref(out_cur_iter)
            )
        )
        return cls(
    ...
    @classmethod
    def from_model_file(
        cls, model_file: Union[str, Path], pred_parameter: Dict[str, Any]
    ) -> "_InnerPredictor":
    ...
    def predict(
        ...
        if isinstance(data, Dataset):
            ...
            _safe_call(
                _LIB.LGBM_BoosterValidateFeatureNames(
                    self._handle, ptr_names, ctypes.c_int(len(data_names)),
                )
            )
        ...
        if isinstance(data, (str, Path)):
            ...
            try:
                ...
                raise TypeError(
                    f"Cannot predict data for type {type(data).__name__}"
                ) from err
            preds, nrow = self.__pred_for_csr(
                csr=csr,
                start_iteration=start_iteration,
    ...
    def __get_num_preds(
        self, start_iteration: int, num_iteration: int, nrow: int, predict_type: int
    ) -> int:
    ...
    def __inner_predict_csr_sparse(
        ...
        Union[List[scipy.sparse.csc_matrix], List[scipy.sparse.csr_matrix]], int
    ]:
    ...
    def __pred_for_csr(
        ...
        if nrow > _MAX_INT32:
            sections = (
                [0]
                + list(np.arange(start=_MAX_INT32, stop=nrow, step=_MAX_INT32))
                + [nrow]
            )
            # __get_num_preds() cannot work with nrow > MAX_INT32, so calculate overall number of predictions piecemeal
            n_preds = [
    ...
    def current_iteration(self) -> int:
        ...
        _safe_call(
            _LIB.LGBM_BoosterGetCurrentIteration(
                self._handle, ctypes.byref(out_cur_iter)
            )
        )
...
class Dataset:
    ...
    def _init_from_ref_dataset(
        self, total_nrow: int, ref_dataset: _DatasetHandle
    ) -> "Dataset":
        ...
        _safe_call(
            _LIB.LGBM_DatasetCreateByReference(
                ref_dataset, ctypes.c_int64(total_nrow), ctypes.byref(self._handle),
            )
        )
        return self
    ...
    def _lazy_init(
        ...
        if isinstance(data, (str, Path)):
            ...
            try:
                ...
                raise TypeError(
                    f"Cannot initialize Dataset from {type(data).__name__}"
                ) from err
        ...
        if isinstance(predictor, _InnerPredictor):
            ...
            self._set_init_score_by_predictor(
                predictor=predictor, data=data, used_indices=None
            )
    ...
    def __init_from_seqs(
        self, seqs: List[Sequence], ref_dataset: Optional[_DatasetHandle]
    ) -> "Dataset":
        """
    ...
    def __init_from_np2d(
        self, mat: np.ndarray, params_str: str, ref_dataset: Optional[_DatasetHandle]
    ) -> "Dataset":
            ...
            if self.reference is not None:
                ...
                if self.used_indices is None:
                    ...
                    used_indices = _list_to_1d_numpy(
                        self.used_indices, dtype=np.int32, name="used_indices"
                    )
    ...
    def save_binary(self, filename: Union[str, Path]) -> "Dataset":
        ...
        """
        _safe_call(
            _LIB.LGBM_DatasetSaveBinary(self.construct()._handle, _c_str(str(filename)))
        )
        return self
    ...
    def set_field(
        ...
        if field_name == "init_score":
            ...
            dtype = (
                np.int32
                if (field_name == "group" or field_name == "position")
                else np.float32
            )
            data = _list_to_1d_numpy(data, dtype=dtype, name=field_name)
    ...
    def set_categorical_feature(
        self, categorical_feature: _LGBM_CategoricalFeatureConfiguration
    ) -> "Dataset":
    ...
    def _set_predictor(self, predictor: Optional[_InnerPredictor]) -> "Dataset":
        ...
        if predictor is None and self._predictor is None:
            return self
        elif isinstance(predictor, _InnerPredictor) and isinstance(
            self._predictor, _InnerPredictor
        ):
        ...
        if self._handle is None:
            ...
            self._predictor = predictor
            self._set_init_score_by_predictor(
                predictor=self._predictor, data=self.data, used_indices=None
            )
        elif (
    ...
    def set_feature_name(
        self, feature_name: _LGBM_FeatureNameConfiguration
    ) -> "Dataset":
        """Set feature name.
    ...
    def set_weight(self, weight: Optional[_LGBM_WeightType]) -> "Dataset":
    ...
    def set_position(self, position: Optional[_LGBM_PositionType]) -> "Dataset":
    ...
    def get_data(self) -> Optional[_LGBM_TrainDataType]:
        ...
        if (
            ...
            if self.data is not None:
                if isinstance(self.data, np.ndarray) or isinstance(
                    self.data, scipy.sparse.spmatrix
                ):
                    ...
                    self.data = np.array(
                        list(self._yield_row_from_seqlist(self.data, self.used_indices))
                    )
                else:
                    _log_warning(
    ...
    def num_feature(self) -> int:
        ...
        if self._handle is not None:
            ret = ctypes.c_int(0)
            _safe_call(_LIB.LGBM_DatasetGetNumFeature(self._handle, ctypes.byref(ret)))
            return ret.value
        else:
        ...
        if self._handle is not None:
            ...
            _safe_call(
                _LIB.LGBM_DatasetGetFeatureNumBin(
                    self._handle, ctypes.c_int(feature_index), ctypes.byref(ret)
                )
            )
            return ret.value
        else:
    ...
    def add_features_from(self, other: "Dataset") -> "Dataset":
        ...
        if other.data is None:
            ...
            if isinstance(self.data, np.ndarray):
                ...
                if isinstance(other.data, np.ndarray) or isinstance(
                    other.data, scipy.sparse.spmatrix
                ):
                ...
                if isinstance(other.data, np.ndarray):
                    self.data = concat(
                        (self.data, pd_DataFrame(other.data)), axis=1, ignore_index=True
                    )
                elif isinstance(other.data, scipy.sparse.spmatrix):
                    self.data = concat(
    ...
    def _dump_text(self, filename: Union[str, Path]) -> "Dataset":
        ...
        """
        _safe_call(
            _LIB.LGBM_DatasetDumpText(self.construct()._handle, _c_str(str(filename)))
        )
        return self


_LGBM_CustomObjectiveFunction = Callable[
    [np.ndarray, Dataset], Tuple[np.ndarray, np.ndarray]
]
_LGBM_CustomEvalFunction = Union[
    Callable[[np.ndarray, Dataset], _LGBM_EvalFunctionResultType],
...
class Booster:
        ...
        if train_set is not None:
            ...
            _safe_call(
                _LIB.LGBM_BoosterCreate(
                    train_set._handle, _c_str(params_str), ctypes.byref(self._handle)
                )
            )
            ...
            if self.__init_predictor is not None:
                _safe_call(
                    _LIB.LGBM_BoosterMerge(self._handle, self.__init_predictor._handle)
                )
            out_num_class = ctypes.c_int(0)
            _safe_call(
                _LIB.LGBM_BoosterGetNumClasses(
                    self._handle, ctypes.byref(out_num_class)
                )
            )
    ...
    def trees_to_dataframe(self) -> pd_DataFrame:
        ...
        def create_node_record(
            ...
            def _get_split_feature(
                tree: Dict[str, Any], feature_names: Optional[List[str]]
            ) -> Optional[str]:
    ...
    def add_valid(self, data: Dataset, name: str) -> "Booster":
        ...
        _safe_call(
            _LIB.LGBM_BoosterAddValidData(self._handle, data.construct()._handle)
        )
    ...
    def reset_parameter(self, params: Dict[str, Any]) -> "Booster":
        ...
        if params_str:
            _safe_call(
                _LIB.LGBM_BoosterResetParameter(self._handle, _c_str(params_str))
            )
        self.params.update(params)
        return self
    ...
    def update(
        ...
        if train_set is not None and not is_the_same_train_set:
            ...
            _safe_call(
                _LIB.LGBM_BoosterResetTrainingData(
                    self._handle, self.train_set.construct()._handle
                )
            )
        ...
        if fobj is None:
            ...
            _safe_call(
                _LIB.LGBM_BoosterUpdateOneIter(self._handle, ctypes.byref(is_finished))
            )
    ...
    def num_model_per_iteration(self) -> int:
        ...
        _safe_call(
            _LIB.LGBM_BoosterNumModelPerIteration(
                self._handle, ctypes.byref(model_per_iter)
            )
        )
    ...
    def num_trees(self) -> int:
        ...
        _safe_call(
            _LIB.LGBM_BoosterNumberOfTotalModel(self._handle, ctypes.byref(num_trees))
        )
    ...
    def eval(
        ...
        name: str,
        feval: Optional[
            Union[_LGBM_CustomEvalFunction, List[_LGBM_CustomEvalFunction]]
        ] = None,
    ...
    def shuffle_models(
        ...
        _safe_call(
            _LIB.LGBM_BoosterShuffleModels(
                self._handle, ctypes.c_int(start_iteration), ctypes.c_int(end_iteration)
            )
        )
        return self
    ...
    def dump_model(
        ...
        ret["pandas_categorical"] = json.loads(
            json.dumps(self.pandas_categorical, default=_json_default_with_numpy)
        )
        return ret
    ...
    def predict(
        ...
        **kwargs
        ...
        predictor = _InnerPredictor.from_booster(
            booster=self, pred_parameter=deepcopy(kwargs),
        )
        if num_iteration is None:
    ...
    def refit(
        ...
        **kwargs
        ...
        _safe_call(
            _LIB.LGBM_BoosterGetLinear(self._handle, ctypes.byref(out_is_linear))
        )
    ...
    def num_feature(self) -> int:
        ...
        _safe_call(
            _LIB.LGBM_BoosterGetNumFeature(self._handle, ctypes.byref(out_num_feature))
        )
    ...
    def __get_eval_info(self) -> None:
        ...
        if self.__need_reload_eval_info:
            ...
            _safe_call(
                _LIB.LGBM_BoosterGetEvalCounts(self._handle, ctypes.byref(out_num_eval))
            )
