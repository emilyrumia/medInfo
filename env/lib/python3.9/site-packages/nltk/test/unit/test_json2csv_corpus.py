...
def test_tweet_hashtag(tmp_path, infile):
    ...
    json2csv_entities(
        infile, outfn, ["id", "text"], "hashtags", ["text"], gzip_compress=False,
    )
    assert files_are_identical(outfn, ref_fn)
...
def test_tweet_media(tmp_path, infile):
    ...
    json2csv_entities(
        infile, outfn, ["id"], "media", ["media_url", "url"], gzip_compress=False,
    )
...
def test_tweet_url(tmp_path, infile):
    ...
    json2csv_entities(
        infile, outfn, ["id"], "urls", ["url", "expanded_url"], gzip_compress=False,
    )
