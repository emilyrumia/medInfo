...
class MPStemmer:
    def __init__(self, check_nonstandard=True, kosakata=None):
        ...
        if kosakata == None:
            ...
            self.kosakata = {kata.lower() for kata in wordlist}
        ...
        self.common_informal_dict = json.loads(
            open(common_informal_dict_path, "r").read()
        )
    ...
    def get_top_n_matching(self, kata, n):
        ...
        """
        dists = [
            {"dist": Levenshtein.distance(kata, x), "word": x} for x in self.kosakata
        ]
    ...
    def check_nonstandard_affixed(kata):
        ...
        if res.startswith(("m", "n", "ng", "ny", "ke")) or res.endswith(
            ("i", "in", "an")
        ):
