...
def remove_prefixes(kata, kosakata, n_removed_suffixes, last_removed):
    ...
    if (is_in_dict(res, kosakata)) or (len(res) <= 5):
        ...
        (res[:2] in ["di", "ke", "se", "ku"])
        and (res[:2] != last_removed)
        and (n_removed_suffixes < 3)
    ):
        ...
        # rule 5: beC1erC... --> be-C1erC..., C1 != {'r'|'l'}  (beterbangan)
            ...
            not (res[2] in ["r", "l"]) and is_consonant(res[2]) and is_consonant(res[5])
        ):
            res = remove_prefixes(res[2:], kosakata, n_removed_suffixes + 1, res[:2])
        ...
        elif (res.startswith("memr") and is_vowel(res[4])) or (
            res.startswith("mem") and is_vowel(res[3])
        ):
            # if res[3] == 'r':
            case1 = "m" + res[3:]
            case2 = "p" + res[3:]
        ...
        elif (res.startswith("pemr") and is_vowel(res[4])) or (
            res.startswith("pem") and is_vowel(res[3])
        ):
            # if res[3] == 'r':
            case1 = "m" + res[3:]
            case2 = "p" + res[3:]
...
def stem(kata, kosakata):
    ...
    """
    if (
        (kata.startswith("be") and kata.endswith(("lah", "an", "i")))
        or (kata.startswith("me") and kata.endswith("i"))
        or (kata.startswith("di") and kata.endswith("i"))
        or (kata.startswith("pe") and kata.endswith("i"))
        or (kata.startswith("te") and kata.endswith("i"))
    ):
