...
class _SeekableRawReader(object):
    ...
    def __init__(
        self, client, bucket, key, version_id=None,
    ):
        self._client = client
        self._bucket = bucket
        self._key = key
    ...
    def _open_body(self, start=None, stop=None):
        ...
        try:
            ...
            response = _get(
                self._client, self._bucket, self._key, self._version_id, range_string,
            )
            ...
            try:
                ...
                response = _get(
                    self._client, self._bucket, self._key, self._version_id, None,
                )
...
class Reader(io.BufferedIOBase):
    ...
    def __init__(
        ...
        self._raw_reader = _SeekableRawReader(
            self._client, bucket, key, self._version_id,
        )
        self._current_pos = 0
...
multipart upload may fail"
        ...
        try:
            partial = functools.partial(
                self._client.create_multipart_upload, Bucket=bucket, Key=key,
            )
    ...
    def close(self):
        ...
        if self._total_bytes and self._upload_id:
            ...
            self._client.put_object(
                Bucket=self._bucket, Key=self._key, Body=b"",
            )
...
class SinglepartWriter(io.BufferedIOBase):
    ...
    def __init__(
        self, bucket, key, client=None, client_kwargs=None, writebuffer=None,
    ):
        _initialize_boto3(self, client, client_kwargs, bucket, key)
    ...
    def close(self):
        ...
        try:
            self._client.put_object(
                Bucket=self._bucket, Key=self._key, Body=self._buf,
            )
        except botocore.client.ClientError as e:
            raise ValueError(
