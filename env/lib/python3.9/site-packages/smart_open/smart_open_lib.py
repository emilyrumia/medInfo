...
def open(
    ...
    if "b" not in mode or explicit_encoding is not None:
        decoded = _encoding_wrapper(
            decompressed, mode, encoding=encoding, errors=errors, newline=newline,
        )
    else:
...
def _shortcut_open(
    uri, mode, compression, buffering=-1, encoding=None, errors=None, newline=None,
):
...
def _encoding_wrapper(fileobj, mode, encoding=None, errors=None, newline=None):
    ...
    fileobj = io.TextIOWrapper(
        fileobj, encoding=encoding, errors=errors, newline=newline, write_through=True,
    )
    return fileobj
